...
public class CreatSoldierBlue : MonoBehaviour
{
    ...
    IEnumerator Create(float time, float delyTime, float spwanTime)
    {
        ...
        while (isCreateSolider)
        {
            ...
            {
                //1 << 3 表示2的3次方,指的是中路
                CreateSmartSolider(SoldierType.soldier1, Start1[0], middleRedTowers, 1 << 3);//中路藍方小兵
                
                CreateSmartSolider(SoldierType.soldier1, Start1[1], LeftRedTowers, 1 << 4);//（左）路藍方小兵
                
                CreateSmartSolider(SoldierType.soldier1, Start1[2], RightRedTowers, 1 << 5);//（右）路藍方小兵
