...
public class ObstacleAvoider : MonoBehaviour
{
    ...
    [SerializeField] DotaObstacle obstacle = null;
    ...
    public List<ObstacleInfo> GetAvoidTargets()
    {
        ...
        List<DotaObstacle> avoidTargets = new List<DotaObstacle>();

        foreach (DotaObstacle ob in ObstacleManager.GetInstance().GetObstaclesExcept(obstacle))
        {
            Vector3 obstacleVec = VectorConvert.XZVector(transform.position, ob.transform.position); 

            float obstacleDist = obstacleVec.magnitude;

            if (obstacleDist > probeLength + ob.GetRadius())
            {
                continue;
            }
            ...
            if (obstacleToProjectionDist < radius + ob.GetRadius())
            {
                avoidTargets.Add(ob);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        foreach (DotaObstacle obstacle in ObstacleManager.GetInstance().GetObstaclesExcept(obstacle))
        {
